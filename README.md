...
installs the code at a specific tag rather than a released wheel. Replace `v1.0.12`
with the tag you want; omitting the `@<tag>` ref (not recommended) installs
whatever the default branch head currently is:

```bash
uv tool install "git+https://github.com/SergioLacerda/sdd-harness@v1.0.12#subdirectory=packages/interfaces/sdd_cli"
cd your-project
sdd install --wizard
sdd init --default

...
class SourceSpecBootstrapper:
    ...
    def _bootstrap_from_markdown(self) -> None:
        ...
        # Write an empty guidelines file — no guideline auto-discovery from docs
        # to avoid picking up IDs that appear in technical spec documents as
        # examples or range references rather than actual guideline definitions.
        guidelines_dsl = self.spec / "guidelines.dsl"
        if not guidelines_dsl.exists():
            guidelines_dsl.write_text(
                "# Generated by SourceSpecBootstrapper\n", encoding="utf-8"
            )

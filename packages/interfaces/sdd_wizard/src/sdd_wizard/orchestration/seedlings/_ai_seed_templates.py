...
GOVERNANCE_INJECT_SCRIPT = '''#!/usr/bin/env python3
...
shape understood by all three platforms. Never blocks (always exits 0).
"""
import json
import subprocess
import sys
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 .claude/sdd-governance-inject.py"
          }
        ]
      }
    ]
...
CODEX_CONFIG_TOML_HOOK_MODE = '''[[hooks.UserPromptSubmit]]

[[hooks.UserPromptSubmit.hooks]]
type = "command"
command = "python3 .codex/sdd-governance-inject.py"
timeout = 10
'''
...
GEMINI_SETTINGS_HOOK_MODE_EXTRA = {
    "BeforeAgent": [
        {
            "hooks": [
                {"type": "command", "command": "python3 .gemini/sdd-governance-inject.py"}
            ]
        }
    ]
}

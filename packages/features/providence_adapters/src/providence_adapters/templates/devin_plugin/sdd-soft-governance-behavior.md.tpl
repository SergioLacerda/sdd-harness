...
This is a small, manually-curated selection of CLI-independent behavioral rules distilled from the source Providence project's `.sdd/agent-instructions.md` and `docs/runtime/protocols/AGENT_RUNTIME_PROTOCOL.md`. It is not auto-generated from those files and does not track them byte-for-byte — see `metadata/provenance.json` for the ruleset version. It intentionally omits anything that depends on a live `sdd` CLI connection (handshake, execution gate, fingerprint checks, `providence runtime status`/`providence governance validate`/`providence test run`/`providence lint run`) — those only apply in Hard/Connected mode, which this plugin does not implement.
